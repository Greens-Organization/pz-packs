  ...
  return (
    <Button size="icon" variant="ghost" className="relative" {...props}>
      ...
      {unreadCount > 0 && (
        <Badge
          variant="destructive"
          className="absolute top-0 right-0 translate-x-1/5 -translate-y-1/5 h-5 min-w-5 p-0 px-0.5 rounded-full empty:h-2.5 empty:min-w-2.5 text-xs flex items-center justify-center"
        >
          {unreadCount}
        </Badge>
      )}
